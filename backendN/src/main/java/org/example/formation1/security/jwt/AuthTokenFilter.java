package org.example.formation1.security.jwt;
...
public class AuthTokenFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
    ...
    if ((path.startsWith("/voyage/") || 
        ...
        && !path.equals("/user/signout")) {
      logger.debug("Skipping JWT authentication for public path: {}", path);
      filterChain.doFilter(request, response);
      return;
    }

    // Only try JWT validation for protected endpoints
    try {
      ...
      if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
        ...
        logger.info("JWT valid pour : {}", username);
      }
    } catch (Exception e) {
      logger.error("Erreur d'authentification sur endpoint protégé : {}", e.getMessage());
    }
